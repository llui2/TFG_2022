...
c     Nodes, Edges 
      integer N, M,z
c     For the graph we want:
c     needs to be even
      parameter (N = 100)
      parameter (z = 4)
c     needs to be a multiple of 4
      parameter (M = z*N/2)
      integer MCTOT, MCINI, SCD
c     montecarlo total steps
      parameter (MCTOT =  30 000)
c     montecarlo initial step good values
      parameter (MCINI = MCTOT/3)
c     save spin configuration every configuration montecarlo steps
      parameter (SCD = 100)
...
c     motecarlo simulation variables
...
c           extract the spin configuratinons every SCD montecarlo steps
            if ((IMC.gt.MCINI).and.(SCD*(IMC/SCD).eq.IMC)) then
...
c     Ploting nseed-seed histograms
