...
C-----------------------------------------------------------------------
      ...
      WRITE(*,*) '>>> METROPOLIS SAMPLE GENERATOR <<<'
...
100   FORMAT (A,I5,A,I5,A,I5,A,F4.2)
      WRITE(*,100)'N =',N,', z = ',z,', C = ',C,', T = ',TEMP
...
C     INITAL RANDOM GRAPH
      !CALL IRG(N,M,NBR,INBR,JJ)
C     RANDOM COUPLING ASSIGNMENT
      !CALL RCA(N,M,p,NBR,INBR,JJ)

C     SPIN CONFIGURATION FILE
...
C           EXTRACT THE SPIN CONFIGURATION EVERY SC MONTE-CARLO STEPS
            IF ((IMC.GT.MCINI).AND.(SC*(IMC/SC).EQ.IMC)) THEN
                  ...
                  CALL ARRAY2BIN(N,bin1,S1)
                  CALL ARRAY2BIN(N,bin2,S2)
